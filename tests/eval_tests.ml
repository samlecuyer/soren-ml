...
let fresh_env () = Env.make None [] []

let loaded_env () =
    let env = fresh_env () in
    C.iter (fun k v -> Env.set env (T.Symbol k) v) Core_sn.ns;
    env

(* let rep str env = (eval (read str));; *)

let test_eval_numbers _ =
